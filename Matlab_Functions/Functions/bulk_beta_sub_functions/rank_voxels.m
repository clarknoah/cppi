function [beta_comp] = rank_voxels(pre_beta, post_beta)
...
for x=1:length(pre_beta(1,:))
    beta_comp.random(3,x) = beta_comp.random(2,x) - beta_comp.random(1,x);
    beta_comp.random(4,x) = x;
end

for x=1:length(pre_beta(1,:))
    beta_comp.sequence(3,x) = beta_comp.sequence(2,x) - beta_comp.sequence(1,x);
    beta_comp.sequence(4,x) = x;
end

beta_comp.comparison = beta_comp.sequence(3,:) - beta_comp.random(3,:);
