...
input_roi_dir = [config.paths.subjects_root '/%s_1/perm_test'];
output_roi_dir = [config.paths.subjects_root '/%s_1/rois'];
hemis = {'lh', 'rh'};

for x=1:length(subs)
  for hemi = hemis
    in_roi_dir = sprintf(input_roi_dir, char(subs(1)));
    out_roi_dir = sprintf(output_roi_dir, char(subs(1)));
   ...
   % save_untouch_nii(BA6_pmv, pmvfn);
    ...
    %roi6 = sprintf('%s/l_sup_parietal.nii,1', out_roi_dir, char(hemi));
    A1 = sprintf('%s/A1.nii,1', out_roi_dir, char(hemi));
    coregToEpi(epi_space, source_image, roi1, roi2);
